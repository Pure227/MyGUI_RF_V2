...
gender = np.random.choice(['man', 'woman'], n_samples)
...
def predict_gender():
    try:
        ...
        result_entry.insert(0, 'man' if prediction == 'man' else 'woman')
    except ValueError:
        result_entry.delete(0, tk.END)
...
# สร้างปุ่มสำหรับเปลี่ยนสีพื้นหลัง
green_button = tk.Button(root, text="Green", command=lambda: change_window_color("green"))
green_button.place(x=10, y=450)

blue_button = tk.Button(root, text="Blue", command=lambda: change_window_color("blue"))
blue_button.place(x=60, y=450)

red_button = tk.Button(root, text="Red", command=lambda: change_window_color("red"))
red_button.place(x=100, y=450)
...
open_csv_button = tk.Button(root, text="Open CSV", bg="red", command=open_csv)
